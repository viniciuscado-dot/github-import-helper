...
interface CopyHistoryFullProps {
  briefings: CopyFormRecord[];
  onBack: () => void;
  onView: (briefing: CopyFormRecord) => void;
  onViewBriefing: (briefing: CopyFormRecord) => void;
  onNewVersion: (briefing: CopyFormRecord) => void;
  onDelete: (id: string) => void;
  onRefresh: () => void;
  canDelete: boolean;
}

export function CopyHistoryFull({
  briefings,
  onBack,
  onView,
  onViewBriefing,
  ...
  const filtered = briefings.filter(b => {
    ...
    if (typeFilter !== 'all' && b.copy_type !== typeFilter) return false;
    ...
    if (keyword) {
      ...
      if (!matchName && !matchResponse) return false;
    }
    return true;
  });
  ...
  useEffect(() => { setCurrentPage(1); }, [clientFilter, typeFilter, creatorFilter, startDate, endDate, keyword]);

  const hasFilters = clientFilter || typeFilter !== 'all' || creatorFilter !== 'all' || startDate || endDate || keyword;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{filtered.length} resultados encontrados</p>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>{briefings.length === 0 ? 'Nenhum briefing encontrado' : 'Nenhum resultado para os filtros aplicados'}</p>
        </div>
      ) : (
        <Card className="bg-card/80 backdrop-blur-sm">
                ...
                <TableBody>
                  {paginated.map(briefing => (
                    <TableRow key={briefing.id} className="group">
                      ...
                      <TableCell>
                        <Badge
                          variant={briefing.status === 'completed' ? 'default' : briefing.status === 'processing' ? 'secondary' : 'destructive'}
                          className={briefing.status === 'completed' ? 'bg-green-500/10 text-green-600 border-green-500/20 dark:text-green-400' : ''}
                        >
                          {briefing.status === 'completed' ? 'Concluído' : briefing.status === 'processing' ? 'Processando' : 'Erro'}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => onViewBriefing(briefing)} title="Ver briefing">
